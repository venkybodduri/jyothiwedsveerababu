(this.workbox = this.workbox || {}),
  (this.workbox.precaching = (function (t, e, n, i, s, c) {
    "use strict";
    try {
    ...
    class h {
      constructor(t) {
        (this.t = e.cacheNames.getPrecacheName(t)),
          (this.i = new Map()),
          (this.s = new Map()),
          (this.o = new Map());
      }
      addToCacheList(t) {
        ...
        for (const n of t) {
          ...
          if (this.i.has(i) && this.i.get(i) !== t)
      ...
      async install({ event: t, plugins: e } = {}) {
        ...
        for (const [t, e] of this.i)
          r.has(e) ? i.push(t) : n.push({ cacheKey: e, url: t });
        const o = n.map(({ cacheKey: n, url: i }) => {
        ...
        await Promise.all(o);
        return { updatedURLs: n.map((t) => t.url), notUpdatedURLs: i };
      }
      async activate() {
      ...
      async h({
        ...
        let l,
          f = await i.fetchWrapper.fetch({ event: o, plugins: a, request: u });
        for (const t of a || []) "cacheWillUpdate" in t && (l = t);
        if (
          !(l
            ? await l.cacheWillUpdate({ event: o, request: u, response: f })
            : f.status < 400)
        )
        ...
        f.redirected && (f = await c.copyResponse(f)),
      ...
      createHandlerBoundToURL(t, e = !0) {
        if (!this.getCacheKeyForURL(t))
          throw new s.WorkboxError("non-precached-url", { url: t });
    ...
    function g(t) {
      l().addToCacheList(t),
        t.length > 0 &&
          (self.addEventListener("install", y),
          self.addEventListener("activate", p));
    }
    return (
      (t.PrecacheController = h),
